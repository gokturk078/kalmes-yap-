...
export default function ServiceCard({ title, description, iconName, index }: ServiceCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group relative p-12 rounded-[3rem] bg-white border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-slate-200/50 hover:-translate-y-2 transition-all duration-700"
    >
      ...
      <div className="relative z-10">
        <div className="w-16 h-16 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-900 mb-10 group-hover:bg-primary group-hover:text-white transition-all duration-700">
          <Icon size={32} strokeWidth={1.5} />
        </div>
        
        <h3 className="text-3xl font-black text-slate-900 mb-6 uppercase tracking-[-0.04em] leading-tight">
          {title}
        </h3>
